...
class Option:
    ...
    def get_rect(self):
        if(self._pos != None):
            font = Option.font.get_font(self._font_str)
            width, height = font.size(self.text)
            height *= 1.25 * len(self.text.splitlines())
            left, top = self._pos[0] - width//2, self._pos[1] - height//2
        ...
        while(self.run_display):
            ...
            if(len(self._options) > 0):
                ...
                for option in self.get_options():
                    ...
                    text_height = option.get_rect().height * 1.25 * len(option.text.splitlines())
