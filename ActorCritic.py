...
CA_conv_flat = tf.concat([tf.reshape(CA_conv, [1, -1]), Caction], 1)

CW1 = tf.Variable(tf.truncated_normal(
    shape=[CA_conv_flat.shape[1], 400], stddev=0.1))
Cb1 = tf.Variable(tf.constant(0.5, shape=[400]))
Cz1 = tf.matmul(CA_conv_flat, CW1) + Cb1
Ca1 = tf.nn.silu(Cz1)

CW2 = tf.Variable(tf.truncated_normal(
    shape=[400, 100], stddev=0.1))
Cb2 = tf.Variable(tf.constant(0.0, shape=[100]))
...
Cupdate = tf.train.AdamOptimizer(1e-4).minimize(Closs)

# actor
Qactions = tf.placeholder(tf.float32, shape=[4])
Aloss = -tf.log(actions) * Qactions
Aupdate = tf.train.AdamOptimizer(1e-4).minimize(Aloss)
...
for i in range(1, 10_001):
    ...
    # for x in [AA_conv , Aa1, Aa2 , Aa3 , Aa4 , Ah]:
