...
W_conv = tf.Variable(tf.truncated_normal(
    shape= [3, 3, 1, 40], stddev= 1e-5))
B_conv = tf.Variable(tf.constant(value= 0.0, shape= [40]))
...
W4 = tf.Variable(tf.truncated_normal(
    shape = [50, 10], stddev=1e-5))
b4 = tf.Variable(tf.constant(0.0, shape=[10]))
z4 = tf.matmul(a3, W4) + b4
a4 = tf.nn.silu(z4)
W5 = tf.Variable(tf.truncated_normal(
    shape = [10, 4], stddev=1e-5))
b5 = tf.Variable(tf.constant(0.0, shape=[4]))
Q4actions = tf.matmul(a4, W5) + b5
